...
$possible_drivers = array("carto");

define("DRIVER", "carto");

class Min_DB {
		var $extension = "Carto", $_link, $_result, $_string, $_database = true, $server_info, $affected_rows, $error;
    ...
    function __carto_query($username, $apikey, $sql) {
			$url = sprintf("http://%s.carto.com/api/v2/sql", addcslashes($username, "'\\"));
			$data = array(
        'api_key' => $apikey,
        'q' => $sql
			);
		...
		function connect($server, $username, $password) {
			...
			$result = $this->__carto_query($this->_username, $this->_apikey, "SHOW all");
			foreach ($result->rows as $row) {
					$this->_link[$row->name] = $row->setting;
			}
			ksort($this->_link);
			if ($this->_link) {
					$this->server_info = $this->_link["server_version"];
			}
			return (bool) $this->_link;
		}

    function quote($string,$includeouterquotes=true) {
		...
		function query($query, $unbuffered = false) {
			$result = @$this->__carto_query($this->_username, $this->_apikey, $query);
			$this->error = "";
			if ($result->error) {
				$this->error = $result->error[0];
				return false;
			} elseif (!$result->fields) {
				$this->affected_rows = $result->total_rows;
				return true;
			}
	...
	class Min_Result {
		var $_result, $_rownum = 0, $_offset = 0, $num_rows;
		...
		function fetch_assoc() {
		  return (array) $this->_result->rows[$this->_rownum++];
		}

		function fetch_row() {
		  return array_values((array) $this->_result->rows[$this->_rownum++]);
		}

		function fetch_field() {
		  $fields = (array) $this->_result;
			$field_names = array_keys($fields);
			$column = $this->_offset++;
			$return = new stdClass;
			$return->name = $field_names[$column];
			$return->orgname = $return->name;
			$return->type = $fields[$return->name]->type;
